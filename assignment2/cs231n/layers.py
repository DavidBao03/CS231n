...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache =(x, x_hat, eps, gamma, x_mean, x_var)
...
def batchnorm_backward(dout, cache):
    ...
    x, x_hat, eps, gamma, mean_x, x_var = cache
    ...
    dx_hat = dout * gamma
    dvar = -0.5 * np.sum(dx_hat * (x - mean_x), axis=0) * np.power(x_var + eps, -1.5)
    dx_mean = -np.sum(dx_hat / np.sqrt(x_var + eps), axis=0) - 2 * dvar * np.sum(x - mean_x, axis=0) / N
    dx = dx_hat / np.sqrt(x_var + eps) + dvar * 2 * (x - mean_x) / N + dx_mean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_hat = dout * gamma
    dx = (1.0 / N) * np.sqrt(x_var + eps) * (N * dx_hat - np.sum(dx_hat, axis=0) - x_hat * np.sum(dx_hat * x_hat, axis=0))
