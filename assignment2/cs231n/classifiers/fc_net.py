...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for idx in range(1, self.num_layers):
            ...
            bn_param = self.bn_params[idx - 1] if self.normalization is not None and idx!=self.num_layers else None

            out, cache[idx] = affine_bn_ln_relu_drop_forward(out, W, b, gamma, beta, bn_param, self.normalization, dropout_param)
        ...
        for idx in range(1, self.num_layers + 1):
            loss += 0.5 * self.reg * np.sum(self.params[f"W{idx}"] ** 2)

        dout, dw, db = affine_backward(dout, cache[self.num_layers])

        grads[f"W{self.num_layers}"] = dw + self.reg * self.params[f"W{self.num_layers}"]
        grads[f"b{self.num_layers}"] = db
        
        for idx in reversed(range(1, self.num_layers)):
            dout, dw, db, dgamma, dbeta = affine_bn_ln_relu_drop_backward(dout, cache[idx])
